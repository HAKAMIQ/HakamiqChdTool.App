...
function Test-ReleaseTrustGates {
    ...
    foreach ($required in @(
        ...
        'Set-MpPreference\s*`\s*\r?\n\s*-DisableRealtimeMonitoring \$false\s*`\s*\r?\n\s*-DisableArchiveScanning \$false',
        '\$status\.AMServiceEnabled -eq \$true',
        '\$status\.AntivirusEnabled -eq \$true',
        '\$status\.RealTimeProtectionEnabled -eq \$true',
        '\$preferences\.DisableRealtimeMonitoring -eq \$false',
    ...
    $orderedSteps = @(
        ...
        'Enable Microsoft Defender release protection',
        'Setup .NET',
