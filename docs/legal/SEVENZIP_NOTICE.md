...
If a release bundles 7-Zip, use the official Windows command-line files and place them here:
...
The project copies `Tools\7zip\**\*` to the build and publish output when those files are present.

If 7-Zip is not available at runtime, the application uses its managed SharpCompress archive path where supported.
