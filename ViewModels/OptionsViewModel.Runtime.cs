...
public sealed partial class OptionsViewModel
{
    private void LoadProcessorChoices()
    {
        ...
        ProcessorOptions.Add(new ProcessorOption(0, ArabicUi.Get("LocAdv_ProcessorAutoRecommendedLabel")));

        foreach (int value in BuildProcessorValues(logicalCount))
        {
            ProcessorOptions.Add(new ProcessorOption(value, value.ToString()));
        }
    }
    ...
    private void LoadConcurrentConversionChoices()
    {
        ...
        {
            ConcurrentConversionOptions.Add(new ProcessorOption(value, value.ToString()));
        }
    }
    ...
    private ChoiceOption? ResolveCompressionPreset(string? compressionValue)
    {
    ...
    private ChoiceOption? ResolvePerformanceMode(ConversionPerformanceMode performanceMode)
    {
    ...
    private ChoiceOption? ResolvePriorityMode(ChdmanProcessPriorityMode priorityMode)
    {
    ...
    private ChoiceOption? ResolveHunkPreset(int hunkSizeBytes) => hunkSizeBytes switch
    {
