...
public partial class MainWindowViewModel
{
        ...
        IDictionary<ArchiveInspectionCacheKey, SevenZipProcessResult>? sevenZipListingCache = null)
    {
        ...
        foreach (string path in paths)
        {
            ...
            string action = ResolveRequestedAction(path, executionProfile);

            if (ArchivePreviewIntakePolicy.BlocksQueuedArchiveProcessing(path, intakeSource))
            {
            ...
            QueuePlatformPresentation platform = BuildQueuePlatformPresentation(path);
