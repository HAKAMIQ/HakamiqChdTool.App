    ...
    int PlatformConfidence)
{
    public bool IsBlocked =>
        Action == MediaInputDecisionAction.Block;

    public bool IsRedirectedToCue =>
        Action == MediaInputDecisionAction.RedirectToCue;

    public bool RequiresTemporaryCue =>
        Action ==
        MediaInputDecisionAction.AcceptStandaloneBinWithTemporaryCue;

    public static MediaInputDecision Accept(
        string path)
    {
        return new MediaInputDecision(
    ...
    public static MediaInputDecision RedirectToCue(
        string originalPath,
        string cuePath)
    {
        return new MediaInputDecision(
    ...
    public static MediaInputDecision Block(
        string path,
        string messageKey)
    {
        return new MediaInputDecision(
            ...
            string.IsNullOrWhiteSpace(messageKey)
                ? MediaInputPolicy.UnsupportedMessageKey
                : messageKey,
            null,
            null,
            0);
