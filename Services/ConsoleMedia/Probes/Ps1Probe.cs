...
internal sealed class PlayStationConsoleDiscProbe : IConsoleDiscIdentityProbe
{
    ...
    {
        if (DiscRawSerialProbe.TryProbe(
                context.Path,
                out DiscRawSerialProbeResult serial))
        {
        ...
        if (context.ContainsText("BOOT2")
            && context.ContainsText("SYSTEM.CNF"))
        {
            return ConsoleDiscIdentityResult.Create(
                "PlayStation 2",
                88,
        ...
        if (context.ContainsText("SYSTEM.CNF")
            && (context.ContainsText("SLUS_")
                || context.ContainsText("SCUS_")
                || context.ContainsText("SLES_")
                || context.ContainsText("SCES_")
                || context.ContainsText("SLPS_")
                || context.ContainsText("SLPM_")))
        {
            return ConsoleDiscIdentityResult.Create(
                "PlayStation 1",
                88,
