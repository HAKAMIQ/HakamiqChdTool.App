...
public static class PlatformDetectionService
{
    ...
    private static readonly ILogger Logger =
        global::Serilog.Log.ForContext(typeof(PlatformDetectionService));

    public static string DetectPlatform(string path) =>
        Detect(path).PlatformName;

    public static bool IsActionablePlatformName(
        string? platformName)
    {
    ...
    public static bool IsOrganizablePlatformName(
        string? platformName)
    {
    ...
    public static bool IsMediaOnlyPlatformName(
        string? platformName)
    {
        if (string.IsNullOrWhiteSpace(platformName))
        {
    ...
    private static bool IsSourceFormatPlatformName(
        string platform)
    {
        return string.Equals(
                platform,
        ...
        string path)
    {
        ...
        try
        {
            ...
            ConversionPathValidator.ThrowIfUnsafeForChdman(
                fullPath,
                nameof(path));
        }
            ...
            || IsExpectedReadException(ex))
        {
            Logger.Debug(
                ex,
                "Platform detection rejected an invalid path. Path={Path}",
                path);
        ...
        if (extension == ".bin")
        {
                ...
                || IsExpectedPathException(ex))
            {
                Logger.Debug(
                    ex,
                    "Platform detection console identity probe failed. Path={Path}",
                    fullPath);
            }
        }

        if (DiscRawSerialProbe.TryDetectPlatform(
                fullPath,
                out PlatformDetectionResult rawSerialDetection))
        {
            return PlatformCandidateFilter.Apply(
                fullPath,
                rawSerialDetection);
            ...
            extension switch
            {
                ".gdi" =>
                    PlatformDetectionResult.Create(
                        ...
                        string.Empty,
                        95,
                        GdiExtensionReasonKey),

                ".toc" =>
                    DetectFromPathKeywords(
                        fullPath,
                        string.Empty),
                ...
                ".cso" =>
                    PlatformDetectionResult.Create(
                        ...
                        string.Empty,
                        95,
                        CsoExtensionReasonKey),

                ".cue" =>
                    DetectFromCue(fullPath),
    ...
    private static PlatformDetectionResult DetectFromCue(
        string cuePath)
    {
        ValidateReadableInputPath(cuePath);

        PlatformDetectionResult pathHint =
            DetectFromPathKeywords(
                cuePath,
                string.Empty);

        if (IsActionablePlatformName(
        ...
        if (DiscMetadataProbe.TryDetectPlatform(
                cuePath,
                out PlatformDetectionResult metadataDetection))
        {
            ...
            || IsExpectedPathException(ex))
        {
            Logger.Debug(
                ex,
                "Platform detection could not parse CUE; using path keywords. Path={Path}",
                cuePath);
    ...
    private static PlatformDetectionResult DetectFromIso(
        string isoPath)
    {
        ...
        try
        {
            ...
            {
                if (TryDetectPs3BluRayIso(
                        isoPath,
                        out PlatformDetectionResult ps3BluRayRawDetection))
                {
                    ...
                    @"\SYSTEM.CNF"))
            {
                        ...
                        StringComparison.OrdinalIgnoreCase))
                {
                    return PlatformDetectionResult.Create(
                        ...
                        string.Empty,
                        75,
                        Ps1SystemCnfReasonKey);
                }
            }

            if (TryDetectPs3BluRayIso(
                    isoPath,
                    out PlatformDetectionResult ps3Detection))
            {
                return ps3Detection;
            }
            ...
            return DetectFromPathKeywords(
                isoPath,
                string.Empty);
        }
        catch (Exception ex) when (
            IsDiscUtilsInvalidFileSystemException(ex))
        {
            Logger.Warning(
            ...
            return DetectFromIsoFallbackWithSafeSize(
                isoPath,
                string.Empty);
        }
        catch (Exception ex) when (
            IsExpectedReadException(ex)
            || IsExpectedPathException(ex)
            || IsDiscUtilsReadException(ex))
        {
            Logger.Debug(
                ex,
                "Platform detection could not read ISO; using fallback. Path={Path}",
                isoPath);
    ...
    private static bool TryDetectPs3BluRayIso(
        string isoPath,
        out PlatformDetectionResult detection)
    {
        detection =
            PlatformDetectionResult.Create(
                string.Empty,
        ...
        try
        {
            ...
            var analyzer =
                new BluRayIsoAnalysisService();

            if (!analyzer.TryAnalyze(
                    isoPath,
            ...
            detection =
                PlatformDetectionResult.Create(
                    ...
                    result.Metadata.HasMinimumRequiredStructure
                        ? 96
                        : 88,
                ...
                or OverflowException)
        {
            Logger.Debug(
                ex,
                "Platform detection could not complete PS3/Blu-ray raw ISO probe. Path={Path}",
                isoPath);
    ...
    private static PlatformDetectionResult
        DetectFromIsoFallbackWithSafeSize(
            string isoPath,
            string fallback)
    {
        try
            ...
            || IsExpectedPathException(ex))
        {
            Logger.Debug(
                ex,
                "Platform detection could not read file size during fallback. Path={Path}",
                isoPath);
    ...
    private static PlatformDetectionResult DetectFromIsoFallback(
        string isoPath,
        long fileSize,
        string fallback)
    {
    ...
    private static bool LooksLikeIso9660Volume(
        string isoPath)
    {
        try
        {
            ...
            return HasIso9660DescriptorAt(
                    stream,
                    0x8001)
                || HasIso9660DescriptorAt(
                    stream,
                    0x8801)
                || HasIso9660DescriptorAt(
                    stream,
            ...
            || IsExpectedPathException(ex))
        {
            Logger.Debug(
                ex,
                "Platform detection ISO9660 signature probe failed. Path={Path}",
                isoPath);
    ...
    private static bool HasIso9660DescriptorAt(
        FileStream stream,
        long offset)
    {
    ...
    private static bool TryDetectDefaultXbe(
        CDReader cd)
    {
        if (IsoFileExists(
                cd,
    ...
    private static bool IsoFileExists(
        CDReader cd,
        string path) =>
        cd.FileExists(
            NormalizeIsoPath(path));

    private static bool IsoDirectoryExists(
        CDReader cd,
        string path) =>
        cd.DirectoryExists(
            NormalizeIsoPath(path));

    private static string CombineIsoPath(
        string directory,
        string fileName)
    {
        string normalizedDirectory =
            NormalizeIsoPath(directory)
                .TrimEnd('\\');
    ...
    private static string NormalizeIsoPath(
        string path)
    {
        if (string.IsNullOrWhiteSpace(path))
        {
    ...
    private static PlatformDetectionResult DetectFromPathKeywords(
        string path,
        string fallback)
    {
        string normalized =
            NormalizePathHint(path);
    ...
    private static string NormalizePathHint(
        string path)
    {
    ...
    private static bool ContainsPathHint(
        string source,
        params string[] values)
    {
        foreach (string value in values)
        {
            ...
            if (ContainsWithTokenBoundaries(
                    source,
                    normalizedValue))
            {
                return true;
    ...
    private static bool ContainsWithTokenBoundaries(
        string source,
        string value)
    {
        int searchIndex = 0;
        ...
        while (searchIndex < source.Length)
        {
            ...
            bool boundaryBefore =
                before < 0
                || char.IsWhiteSpace(source[before]);

            bool boundaryAfter =
                after >= source.Length
                || char.IsWhiteSpace(source[after]);
    ...
    private static string? ParseCueFileReference(
        string line) =>
        CueSheetFileStatementReader.TryRead(
            line,
            out string fileName,
            out _)
            ? fileName
    ...
    private static IEnumerable<string> ReadTextLines(
        string path)
    {
        ...
        if (!info.Exists
            || info.Length <= 0
            || info.Length > MaxDescriptorTextBytes)
        {
            throw new IOException(
                "Descriptor size is outside the supported platform-detection scan bounds.");
        }
    ...
    private static string ReadTextFileFromIso(
        CDReader cd,
        string path)
    {
        using Stream stream =
    ...
    private static bool IsReadableInputPath(
        string path)
    {
        try
        {
    ...
    private static void ValidateReadableInputPath(
        string path)
    {
    ...
    private static bool IsSameDirectoryChild(
    ...
    private static string TrimDirectorySeparators(
        string path)
    {
        string? root =
    ...
    private static bool IsDiscUtilsInvalidFileSystemException(
        Exception ex) =>
        string.Equals(
            ex.GetType().FullName,
            "DiscUtils.InvalidFileSystemException",
            StringComparison.Ordinal)
        || string.Equals(
            ex.GetType().Name,
    ...
    private static bool IsDiscUtilsReadException(
        Exception ex) =>
        ex.GetType().FullName?.Contains(
            "DiscUtils",
