...
internal static class DiscSerialCatalog
{
    ...
    private static readonly Dictionary<int, DiscSerialPrefixRule> PrefixRules =
        BuildPrefixRules();
        ...
        out DiscSerialCatalogResult result)
    {
        ...
        for (int index = 0;
             index <= span.Length - SerialPrefixLength;
             index++)
        {
            ...
            int position =
                index + SerialPrefixLength;

            SkipSerialSeparators(
                span,
                ref position);
            ...
            if (includeOptionalTail)
            {
                ...
                if (TryReadExactDigits(
                        span,
                        ref position,
                        2,
                        out string resolvedTail))
                {
                    tail = resolvedTail;
                }
                else
            ...
            string platform =
                string.IsNullOrWhiteSpace(rule.PlatformOverride)
                    ? fallbackPlatform
                    : rule.PlatformOverride;

            if (string.IsNullOrWhiteSpace(platform)
                || string.IsNullOrWhiteSpace(rule.Region))
            {
                continue;
            }
        ...
        out DiscSerialPrefixRule rule)
    {
        ...
        if (index < 0
            || index + SerialPrefixLength > text.Length)
        {
            return false;
        }
        ...
        if (!PrefixRules.TryGetValue(
                key,
                out DiscSerialPrefixRule candidate))
        {
            return false;
        }
        ...
        bool allowed = profile switch
        {
            DiscSerialScanProfile.Metadata =>
                candidate.AllowMetadataProbe,

            DiscSerialScanProfile.Raw =>
                candidate.AllowRawProbe,
    ...
    private static Dictionary<int, DiscSerialPrefixRule>
        BuildPrefixRules()
    {
    ...
    private static void Add(
        ...
        bool raw)
    {
        rules[BuildPrefixKey(prefix.AsSpan())] =
            new DiscSerialPrefixRule(
                prefix,
                region,
                platformOverride,
                metadata,
    ...
    private static int BuildPrefixKey(
        ReadOnlySpan<char> prefix)
    {
        ...
        {
            char value =
                char.ToUpperInvariant(prefix[index]);
    ...
    private static void SkipSerialSeparators(
        ReadOnlySpan<char> text,
        ref int position)
    {
        while (position < text.Length
               && IsSerialSeparator(text[position]))
        {
            position++;
        }
    }

    private static bool IsSerialSeparator(
        char value)
    {
    ...
    private static bool IsSerialBoundaryBefore(
        ReadOnlySpan<char> text,
        int index)
    {
        return index == 0
    ...
    private static bool IsSerialBoundaryAfter(
        ReadOnlySpan<char> text,
        int index)
    {
