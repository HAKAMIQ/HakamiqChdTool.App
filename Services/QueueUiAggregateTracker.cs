...
internal readonly record struct QueueUiAggregateSnapshot(
    int TotalCount,
    int WaitingCount,
    int ActiveCount,
    int CompletedCount,
...
internal sealed class QueueUiAggregateTracker
{
    private readonly object _gate = new();
    private readonly Dictionary<Guid, QueueUiAggregateRowState> _index = new();
    ...
    public QueueUiAggregateSnapshot Capture()
    {
        lock (_gate)
        {
            return new QueueUiAggregateSnapshot(
                _totalCount,
                _waitingCount,
                _activeCount,
                _completedCount,
    ...
    public void Rebuild(IEnumerable<QueueRowData> rows)
    {
        ...
        lock (_gate)
        {
            ...
            {
                QueueUiAggregateRowState state = BuildRowState(row);
                if (!state.IsTracked)
                {
                    continue;
                }
    ...
    public void Upsert(QueueRowData row)
    {
        lock (_gate)
        {
            QueueUiAggregateRowState next = BuildRowState(row);
            _index.TryGetValue(row.ItemId, out QueueUiAggregateRowState previous);
    ...
    public void Remove(QueueRowData row)
    {
        lock (_gate)
        {
            if (_index.TryGetValue(row.ItemId, out QueueUiAggregateRowState existing))
            {
                _index.Remove(row.ItemId);
    ...
    private static QueueUiAggregateRowState BuildRowState(QueueRowData row)
    {
        ...
        return new QueueUiAggregateRowState(
            true,
    ...
    private void ApplyDelta(QueueUiAggregateRowState previous, QueueUiAggregateRowState next)
    {
    ...
    private readonly record struct QueueUiAggregateRowState(
        bool IsTracked,
        bool IsWaiting,
        bool IsActive,
        bool IsCompleted,
