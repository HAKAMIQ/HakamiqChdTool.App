...
internal sealed class AppSessionCoordinator : IAppSessionCoordinator, IDisposable
{
    private readonly QueueController _queueController;
    private readonly IAppSessionUiPort _ui;
    ...
    public AppSessionCoordinator(QueueController queueController, IAppSessionUiPort ui)
    {
        _queueController = queueController ?? throw new ArgumentNullException(nameof(queueController));
    ...
    public async Task ProcessSelectedAsync(TaskQueueItemViewModel? selected)
    {
        ...
        if (!TryBeginSession(out CancellationToken sessionToken))
        {
            return;
        }
        ...
        try
        {
            ...
            _ui.SetHasActiveQueueBindingAndSync(selected, true);
            activeBindingSet = true;

            bool wasCancelled = await _queueController
                .RunBatchAsync(new[] { chd }, sessionToken)
                .ConfigureAwait(true);

            PostConversionArtifactResult sessionArtifacts = PostConversionArtifactResult.Empty;
            if (!wasCancelled)
            {
                sessionArtifacts = await _ui.GenerateM3uPlaylistsForCompletedChdOutputsAsync(
                        CollectCompletedChdWorkflowOutputs(new[] { chd }))
                    .ConfigureAwait(true);
            }

            _ui.BuildRunSummary(showCompletionDialog: false, wasCancelled: wasCancelled, processedSelectionOnly: true, sessionArtifacts);
        }
        catch (OperationCanceledException)
        {
            _ui.BuildRunSummary(showCompletionDialog: false, wasCancelled: true, processedSelectionOnly: true, PostConversionArtifactResult.Empty);
        }
        catch (Exception ex)
        {
            _ui.SetFooterStatus(MainWindowMessages.ProcessSelectedFailedFooter);
        ...
        finally
        {
            if (activeBindingSet)
            {
                _ui.SetHasActiveQueueBindingAndSync(selected, false);
            }
    ...
    public async Task VerifySelectedChdAsync(TaskQueueItemViewModel? selected)
    {
        ...
        try
        {
            ...
            _ui.BuildRunSummary(showCompletionDialog: false, wasCancelled: wasCancelled, processedSelectionOnly: true, PostConversionArtifactResult.Empty);
        }
        catch (OperationCanceledException)
        {
            _ui.BuildRunSummary(showCompletionDialog: false, wasCancelled: true, processedSelectionOnly: true, PostConversionArtifactResult.Empty);
    ...
    private async Task IngestForSelectedModeAsync(IEnumerable<string> paths, QueueIngestKind inputKind, QueueIntakeSource intakeSource)
    {
        QueueExecutionProfile profile = _ui.GetSelectedInputExecutionProfile();
    ...
    private async Task RunQueuedItemsByIdsAsync(IReadOnlyList<Guid> itemIds, bool processedSelectionOnly)
    {
        ...
        try
        {
            ...
            foreach (TaskQueueItemViewModel vm in queuedVms)
            {
                ...
                {
                    _ui.BuildRunSummary(showCompletionDialog: false, wasCancelled: true, processedSelectionOnly: processedSelectionOnly, PostConversionArtifactResult.Empty);
                    return;
                }

                if (string.IsNullOrWhiteSpace(vm.SourcePath))
                {
                    _ui.SetHasActiveQueueBindingAndSync(vm, false);
                    continue;
                }

                await _ui.ApplyAutoStandardizedNameIfEnabledAsync(vm).ConfigureAwait(true);

                _ui.SetHasActiveQueueBindingAndSync(vm, true);
            ...
            bool wasCancelled = await _queueController.RunBatchAsync(batch, sessionToken).ConfigureAwait(true);
            PostConversionArtifactResult sessionArtifacts = PostConversionArtifactResult.Empty;
            if (!wasCancelled)
            {
                sessionArtifacts = await _ui.GenerateM3uPlaylistsForCompletedChdOutputsAsync(
            ...
            _ui.BuildRunSummary(showCompletionDialog: false, wasCancelled: wasCancelled, processedSelectionOnly: processedSelectionOnly, sessionArtifacts);
        }
        catch (OperationCanceledException)
        {
            _ui.BuildRunSummary(showCompletionDialog: false, wasCancelled: true, processedSelectionOnly: processedSelectionOnly, PostConversionArtifactResult.Empty);
    ...
    private void ClearActiveQueueBindings(IEnumerable<TaskQueueItemViewModel> items)
    {
        ...
        {
            _ui.SetHasActiveQueueBindingAndSync(item, false);
        }
    }
    ...
    private void RaiseStateChanged()
    {
        ...
        {
            Log.Debug(ex, "AppSessionCoordinator.SessionStateChanged subscriber threw.");
        }
    }
