...
internal sealed class MainWindowStartupCoordinator
{
    ...
    private Task StartRedumpAutoSyncIfConfiguredAsync(CancellationToken cancellationToken)
    {
        ...
        return Task.Run(
            async () =>
            {
                ...
                {
                    await PersistRedumpAutoSyncTimestampAsync(result.SyncedAtUtc.Value, cancellationToken)
                        .ConfigureAwait(false);
                }
    ...
    private async Task PersistRedumpAutoSyncTimestampAsync(DateTimeOffset syncedAtUtc, CancellationToken cancellationToken)
    {
        if (IsOwnerDispatcherUnavailable())
        {
            return;
